...
export default function InvoiceViewPage() {
    ...
    useEffect(() => {
        getInvoice(id).then(res => {
            ...
            setLoading(false);
        }).catch(() => { toast.error("Erreur"); navigate("/factures"); });
    }, [id, navigate]);

    const updateStatus = (s) => {
        updateInvoice(id, { payment_status: s }).then(() => {
    ...
    const updatePaid = () => {
        updateInvoice(id, { paid_amount: parseFloat(paidAmount) || 0 }).then(r => {
            setInvoice(r.data);
            toast.success("Mis à jour");
        }).catch(() => toast.error("Erreur"));
    };
    ...
    const markPaid = () => {
        updateInvoice(id, { payment_status: "paye", paid_amount: invoice.total_ttc }).then(() => {
            setInvoice(p => ({ ...p, payment_status: "paye", paid_amount: p.total_ttc }));
            toast.success("Payée");
        }).catch(() => toast.error("Erreur"));
    };

    const downloadPdf = () => downloadInvoicePdf(invoice.id, invoice.invoice_number).catch(() => toast.error("Erreur"));
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6" data-testid="invoice-detail-page">
            ...
            <Card>
                <CardHeader><CardTitle className="font-mono">{invoice.invoice_number}</CardTitle></CardHeader>
                <CardContent>
