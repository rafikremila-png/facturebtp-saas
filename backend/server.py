from fastapi import FastAPI, APIRouter, HTTPException, Depends, UploadFile, File, status, Request, Body, Query
...
try:
    ...
    from app.api.routes import service_requests as pg_service_requests_routes
    from app.api.routes import financial_dashboard as pg_financial_dashboard_routes
...
async def get_subscription_status(user: dict = Depends(get_current_user)):
    ...
    subscription_service = get_stripe_subscription_service(db)
    try:
        status = await subscription_service.get_user_subscription(user["id"])
...
async def create_checkout_session(
    ...
    if plan_id not in ["essentiel", "pro", "business"]:
    ...
    try:
        # Create checkout session with plan price
        checkout_request = CheckoutSessionRequest(
            amount=float(plan_config["price_monthly"]),
            currency="eur",
    ...
    try:
        ...
        if status.payment_status == "paid" and transaction.get("payment_status") != "paid":
            ...
            await subscription_service.activate_subscription(
                ...
                stripe_customer_id=transaction.get("user_email", ""),
        ...
        return {
            ...
            "plan_name": PLANS.get(transaction.get("plan_id", ""), {}).get("name", "")
        }
...
async def stripe_webhook(request: Request):
    ...
    try:
        ...
        if not user_id:
            logger.warning(f"Webhook without user_id: {event_type}")
            return {"status": "ignored", "reason": "No user_id in metadata"}
        
        subscription_service = get_stripe_subscription_service(db)
        
        if event_type in ["checkout.session.completed", "payment_intent.succeeded"]:
            if webhook_response.payment_status == "paid" and plan_id:
                await subscription_service.activate_subscription(
                    user_id=user_id,
                    plan=plan_id,
                    stripe_subscription_id=session_id,
                    stripe_customer_id=metadata.get("user_email", ""),
                    current_period_end=(datetime.now(timezone.utc) + timedelta(days=30)).isoformat()
                )
                ...
                logger.info(f"Subscription activated via webhook: {user_id} -> {plan_id}")
        
        elif event_type == "invoice.payment_failed":
            await subscription_service.handle_payment_failed(user_id)
...
async def cancel_subscription_endpoint(user: dict = Depends(get_current_user)):
    ...
    if status.plan in ["trial", "trial_pending"]:
        raise HTTPException(status_code=400, detail="Pas d'abonnement actif à annuler")
    
    if status.status == "canceled":
        raise HTTPException(status_code=400, detail="Abonnement déjà annulé")
    
    success = await subscription_service.cancel_subscription(user["id"])
...
@api_router.get("/subscription/features/{feature}")
async def check_feature_access_endpoint(
    ...
    subscription_service = get_stripe_subscription_service(db)
    access = await subscription_service.check_feature_access(user["id"], feature)
...
# ============== POSTGRESQL ROUTES (New modular architecture) ==============
# Import and include PostgreSQL-based API routers
if PG_ROUTES_AVAILABLE:
    try:
        ...
        app.include_router(pg_service_requests_routes.router, prefix="/api")
        app.include_router(pg_financial_dashboard_routes.router, prefix="/api")
